...
import { Workflow, Plus, Zap, ChevronRight, Pencil } from "lucide-react";
  ...
  return (
    <div>
      <PageHeader eyebrow="Inteligencja" title="Automatyzacje" subtitle="Reguły IF / THEN — twój dom myśli za ciebie."
        action={
          <button onClick={() => navigate("/automations/new")} className="flex items-center gap-2 rounded-2xl bg-gradient-primary px-4 py-2 text-sm text-primary-foreground shadow-glow">
            <Plus className="h-4 w-4" /> Nowa automatyzacja
          </button>
        } />

      <div className="grid grid-cols-1 gap-3 px-5 pb-6 lg:grid-cols-2 lg:px-10">
        {items.map((a) => (
          <div key={a.id} className={`premium-card group transition ${a.enabled ? "" : "opacity-70"}`}>
            <div className="flex items-start justify-between gap-3">
              <button onClick={() => navigate(`/automations/${a.id}`)} className="flex flex-1 items-start gap-3 text-left">
